<h2 align=center>Gensyn Testnet Node Guide by CryptoThrone</h2>

Welcome to the Gensyn Testnet Node setup guide by CryptoThrone! This repository provides a one-command setup to run a Gensyn Testnet Node using the rl-swarm project.

## Installation
3. **Run the Node with One-Command**:
   ```bash
   cd $HOME && rm -rf gensyn-testnet && git clone https://github.com/CryptoThrone/gensyn-testnet.git && chmod +x gensyn-testnet/gensyn.sh && ./gensyn-testnet/gensyn.sh
